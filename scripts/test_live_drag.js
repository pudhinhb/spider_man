...
setTimeout(async () => {
  try {
    const listRes = await fetch('http://localhost:9254/json');
    ...
    ws.onopen = async () => {
      ...
      // Click arrow to go from Spread 1 -> Spread 2
      await send('Runtime.evaluate', {
        expression: `document.querySelector('.sb-turn-arrow.bottom-right').click();`
      });
      await new Promise(r => setTimeout(r, 1500));
      ...
      console.log('Book state before drag:', rectRes.result.value);
      ...
      const dragX = Math.round(r.left + r.width * 0.60);
      ...
      console.log('Active state during drag:', JSON.stringify(activeState.result.value, null, 2));

      // Capture screenshot WHILE MOUSE IS STILL PRESSED
      const shot = await send('Page.captureScreenshot', { format: 'png' });
      fs.writeFileSync(path.join(brainDir, 'verified_active_drag_spread2.png'), Buffer.from(shot.data, 'base64'));
      console.log('Saved verified_active_drag_spread2.png');
